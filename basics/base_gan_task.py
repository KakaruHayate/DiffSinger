...
class BaseGanTask(pl.LightningModule):
    """
    ...
    def build_discriminator(self):
        ...
        self.print_arch(self.discriminator, model_name='discriminator')

    @rank_zero_only
    def print_arch(self, model, model_name='model'):
        utils.print_arch(model, model_name)
    ...
    def on_train_epoch_start(self):
        ...
        # print("All of scheduler:", [s.name for s in self.lr_schedulers()])
        if self.training_sampler is not None:
            self.training_sampler.set_epoch(self.current_epoch)

    def get_lr(self, scheduler_name):
        for scheduler in self.lr_schedulers():
            if hasattr(scheduler, 'name') and scheduler.name == scheduler_name:
                return scheduler.get_last_lr()[0]
            elif (isinstance(scheduler, dict) and 
                  scheduler.get('name') == scheduler_name):
                return scheduler['scheduler'].get_last_lr()[0]
    ...
    def _training_step(self, sample):
        ...
        return {**losses, 'batch_size': float(sample['size'])}

    def training_step(self, sample, batch_idx):
        log_outputs = self._training_step(sample)

        # logs to progress bar
        #gen_lr = self.get_lr("generator_scheduler")
        #d_lr = self.get_lr("discriminator_scheduler")
        self.log_dict(log_outputs, prog_bar=True, logger=False, on_step=True, on_epoch=False)
        self.log('lr', self.lr_schedulers()[0].get_last_lr()[0], prog_bar=True, logger=False, on_step=True, on_epoch=False)
        self.log('lr', self.lr_schedulers()[1].get_last_lr()[0], prog_bar=True, logger=False, on_step=True, on_epoch=False)
        #log_outputs_with_lr = {**log_outputs, "gen_lr": gen_lr, "d_lr": d_lr}
        #self.log_dict(log_outputs_with_lr, prog_bar=True, logger=Falseon_step=True, on_epoch=False)
        # logs to tensorboard
        if self.global_step % hparams['log_interval'] == 0:
            tb_log = {f'training/{k}': v for k, v in log_outputs.items()}
            tb_log['training/g_lr'] = self.lr_schedulers()[0].get_last_lr()[0]
            tb_log['training/d_lr'] = self.lr_schedulers()[1].get_last_lr()[0]
            self.logger.log_metrics(tb_log, step=self.global_step)
        #if self.global_step % hparams['log_interval'] == 0:
        #    tb_log = {f'training/{k}': v for k, v in log_outputs.items()}
        #    tb_log.update({'training/gen_lr': gen_lr, 'training/d_lr': d_lr})
        #    self.logger.log_metrics(tb_log, step=self.global_step)
    ...
    def build_scheduler(self, optimizer, scheduler_args):
    ...
    def configure_optimizers(self):
        optm = self.build_optimizer(self.model, hparams['optimizer_args'])
        d_optm = self.build_optimizer(self.discriminator, hparams['d_optimizer_args'])
        scheduler = self.build_scheduler(optm, hparams['lr_scheduler_args'])
        d_scheduler = self.build_scheduler(d_optm, hparams['d_lr_scheduler_args'])
        # if scheduler is None:
        # return [optm, d_optm], [scheduler, d_scheduler]
        return [
            {
                "optimizer": optm,
                "lr_scheduler": {
    ...
    def on_load_checkpoint(self, checkpoint):
        ...
        d_optimizer_args = hparams['d_optimizer_args']
        scheduler_args = hparams['lr_scheduler_args']
        d_scheduler_args = hparams['d_lr_scheduler_args']
        ...
        if checkpoint.get('lr_schedulers', None):
            ...
            checkpoint['lr_schedulers'][1] = simulate_lr_scheduler(
                d_optimizer_args, d_scheduler_args,
                step_count=checkpoint['global_step'],
                num_param_groups=len(checkpoint['optimizer_states'][1]['param_groups'])
            )
