...
from basics.base_gan_task import BaseGanTask
from modules.losses import DurationLoss, DiffusionLoss, RectifiedFlowLoss, DurGanLoss
from modules.metrics import (
    RawCurveAccuracy, RawCurveR2Score, RhythmCorrectness, PhonemeDurationAccuracy
)
from modules.toplevel import DiffSingerVariance
from modules.fastspeech.tts_modules import DurationDiscriminator
...
class VarianceTask(BaseGanTask):
    def __init__(self):
        super().__init__()
        ...
        self.clip_grad_norm = hparams['clip_grad_norm']
    ...
    def _build_discriminator(self):
        ...
        return DurationDiscriminator(
            offset=dur_hparams['log_offset'],
            in_channels=hparams['hidden_size'],
            filter_channels=dur_hparams['disc_hidden_size'],
            kernel_size=dur_hparams['disc_kernel_size'],
            p_dropout=dur_hparams['disc_dropout']
        )
    ...
    def build_losses_and_metrics(self):
        if self.predict_dur:
            ...
            self.dur_g_loss = DurGanLoss(loss_mode='G_loss')
            self.dur_d_loss = DurGanLoss(loss_mode='D_loss')
            self.register_validation_loss('dur_loss')
            self.register_validation_loss('dur_g_loss')
            self.register_validation_loss('dur_d_r_loss')
            self.register_validation_loss('dur_d_g_loss')
    ...
    def run_model(self, sample, infer=False, val=False):
        ...
        if infer:
            ...
            # 2.然后先进行判别器的训练
            y_dur_hat_r, y_dur_hat_g = self.discriminator(
                dur_pred['hidden_x'].detach(), 
                dur_pred['x_mask'].detach(), 
                ph_dur.detach(), 
                dur_pred['dur_pred'].detach()
            )
            D_r_loss, D_g_loss = self.dur_d_loss(y_dur_hat_r, y_dur_hat_g)
            losses['dur_d_r_loss'] = D_r_loss
            losses['dur_d_g_loss'] = D_g_loss
            D_total_loss = D_r_loss + D_g_loss
            if not val:
                d_optm.zero_grad()
                self.manual_backward(D_total_loss)
                if self.clip_grad_norm is not None:
                    self.clip_gradients(d_optm, gradient_clip_val=self.clip_grad_norm, gradient_clip_algorithm="norm")
                d_optm.step()
                scheduler_d.step()
            ...
            G_r_loss = self.dur_g_loss(y_dur_hat_r, y_dur_hat_g)
            G_total_loss += G_r_loss
            losses['dur_g_loss'] = G_r_loss

            # 3.2.然后是其他loss
            if dur_pred is not None:
                dur_loss = self.lambda_dur_loss * self.dur_loss(dur_pred['dur_pred'], ph_dur, ph2word=ph2word)
                G_total_loss += dur_loss
            ...
            if variances_pred is not None:
                if self.diffusion_type == 'ddpm':
                    ...
                    raise ValueError(f"Unknown diffusion type: {self.diffusion_type}")
                var_loss = self.lambda_var_loss * var_loss
                G_total_loss += var_loss
                losses['var_loss'] = var_loss
            ...
            if not val:
                ...
                if self.clip_grad_norm is not None:
                    self.clip_gradients(optm, gradient_clip_val=self.clip_grad_norm, gradient_clip_algorithm="norm")
                optm.step()
                scheduler_g.step()
    ...
    def _validation_step(self, sample, batch_idx):
        losses = self.run_model(sample, infer=False, val=True)
